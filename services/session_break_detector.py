...
Data source : OANDA v20 – M5 candles (mid OHLC, UTC open time)
        ...
        for symbol in symbols:
            try:
                if symbol not in candle_cache:
                    logger.debug(
                        "%s[SESSION_TRACK] user=%s symbol=%s fetching_candles count=%d provider=OANDA",
                        LP, user_id, symbol, SessionSweepConfig.CANDLE_COUNT,
                    )
                    candles = oanda_provider.get_candles(
